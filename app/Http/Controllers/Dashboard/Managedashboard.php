...
{  
	...
	public function index(Request $request)
    {
		...
		if($dashboard == 0)
		{
		    $dashboard_view = "dashboard.dashboard_manage";
		    $footer			= 'footer';
		...
		$page_data['configdata'] 			= $this->getDatas(); 
		//echo '<pre>'; print_r($this->getDatas()); echo '</pre>'; exit;


        return view($dashboard_view, $page_data); 
    }
	public function getDatas(){ 
		$model = new Tblconfiguration();
	    return $model->getConfigrationdata();
	}
    ...
    public function updateDatas(Request $request){
		...
		$clmn= "value";
		if (empty($val) || empty($uid)) {
			echo json_encode(["status" => "NOTOK", "msg" => "Invalid Input"]);
			return;
		}
	    ...
	    if(!empty($clmn) && !empty($uid)){
	        ...
	        $datamsg = "OK";
	        $msg = "Menu serial updated successfully! $val"; 
	    }else{
	        $datamsg = "NOTOK";
    ...
    public function updateviewDatas(Request $request){
	    $clmn="view_action";
	    $val= $request->get('val');
    ...
    public function deleteDatas(Request $request)
    {
        ...
        DashboardMenu::whereIn('id', $uidexplode)->delete();

        return response()->json([
            'stts' => 'OK',
            'msg'  => 'Menu(s) deleted successfully!'
        ]);
    }
