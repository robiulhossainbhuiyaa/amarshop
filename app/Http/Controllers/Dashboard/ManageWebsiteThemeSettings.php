...
class ManageWebsiteThemeSettings extends Controller 
{ 
	//theme_settings
	...
	public function index(Request $request)
    {
		...
		if($dashboard == 0)
		{
		    $dashboard_view = "dashboard.managewebsiteThemesattings";
		    $footer			= 'footer';
		...
		$page_data['pagedata'] 	= $modelControl->getDashboardIdByData();
	...
	public function updateserialDatas(Request $request){
	    ...
	    $model = new Controlweb();
	    if(!empty($clmn) && !empty($uid)){
	        $model->updateData($clmn, $val, $uid);
	        $datamsg = "OK";
	        $msg = "Themes Update id : $uid / $val";
	    }else{
	        $datamsg = "NOTOK";
	        $msg = "Themes NOT UPDATED : $uid / $val";
	    }
